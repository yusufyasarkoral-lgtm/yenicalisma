...
export default function AgencyPortal(){
  ...
  function send(){
    ...
    if(step.key==='amount'&&!Number(text.replace(/[^0-9,.-]/g,'').replaceAll('.','').replace(',','.'))){setError('Lütfen tutarı sayı olarak yazın.');return}
    setError('');setMessages(current=>[...current,{from:'user',text}]);setAnswers(current=>({...current,[step.key]:text}));setValue('');
    const next=steps[Object.keys(answers).length+1];if(next)setTimeout(()=>setMessages(current=>[...current,{from:'bot',text:next.question}]),250);
  }
