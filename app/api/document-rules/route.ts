...
export async function PUT(request:Request){
 ...
 try{await database().prepare('INSERT INTO document_rules(branch,categories,updated_at) VALUES (?,?,?) ON CONFLICT(branch) DO UPDATE SET categories=excluded.categories,updated_at=excluded.updated_at').bind(q.branch,JSON.stringify([...new Set(q.categories)]),new Date().toISOString()).run();return Response.json({ok:true})}catch{return Response.json({error:'Liste kaydedilemedi'},{status:503})}
}
