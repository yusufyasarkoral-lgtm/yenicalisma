...
export async function GET(request:Request){
 ...
 try{const db=database();const quote=await db.prepare('SELECT * FROM quotes WHERE id=? AND owner_id=?').bind(id,user.userId).first<any>();if(!quote)return error('İş bulunamadı',404);
 ...
 const rule=await db.prepare('SELECT categories FROM document_rules WHERE branch=? AND owner_id=?').bind(quote.branch,user.userId).first<{categories:string}>();const required=rule?JSON.parse(rule.categories):[];
...
export async function POST(request:Request){
 ...
 try{const db=database();if(!await db.prepare('SELECT id FROM quotes WHERE id=? AND owner_id=?').bind(quoteId,user.userId).first())return error('Önce teklif kaydını oluşturun.',404);
...
export async function PATCH(request:Request){
 ...
 result=await db.prepare("UPDATE documents SET extracted_text=?,read_status=?,read_note=?,review_status='pending' WHERE id=? AND EXISTS (SELECT 1 FROM quotes WHERE quotes.id=documents.quote_id AND owner_id=?)").bind(q.text,status,q.note,q.id,user.userId).run();
 }else if(q.action==='category'){
 if(!categories.includes(q.category))return error('Geçersiz evrak türü');result=await db.prepare("UPDATE documents SET category=?,review_status='pending' WHERE id=? AND EXISTS (SELECT 1 FROM quotes WHERE quotes.id=documents.quote_id AND owner_id=?)").bind(q.category,q.id,user.userId).run();
 }else if(q.action==='review'){
 if(!['pending','accepted','needs_attention'].includes(q.status))return error('Geçersiz kontrol durumu');result=await db.prepare('UPDATE documents SET review_status=? WHERE id=? AND EXISTS (SELECT 1 FROM quotes WHERE quotes.id=documents.quote_id AND owner_id=?)').bind(q.status,q.id,user.userId).run();
